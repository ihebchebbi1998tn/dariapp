...
import { Check, ChevronLeft, Upload } from 'lucide-react-native';
...
export default function EditPropertyScreen() {
    ...
    const fetchProperty = async () => {
      try {
        ...
        setForm({
          ...
          workstations: data.workstations?.toString() || '1',
          meeting_rooms: data.meeting_rooms?.toString() || '0',
  ...
  const handleSubmit = async () => {
    ...
    if (!user || !id) {
      Alert.alert('Erreur', 'Vous devez être connecté et avoir un ID de propriété valide.');
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        'Succès',
        'Votre propriété a été mise à jour avec succès.',
        [{ text: 'OK', onPress: () => router.back() }]
      );
    } catch (error) {
      console.error('Error updating property:', error);
      Alert.alert('Erreur', 'Une erreur est survenue lors de la mise à jour de la propriété.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (initialLoading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Chargement des détails de la propriété...</Text>
      </View>
    );
  }
